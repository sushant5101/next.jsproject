...
export default function Home() {
  ...
  return (
    <div className="border-1 border-gray-500 p-2 rounded-md ml-auto mr-auto m-2 w-[60%]">
      ...
      <form className={`mt-1 flex gap-2 ${afterRadio}`}>
        <label htmlFor="guessed_letter">Enter your Letter:</label> <input disabled={!RadioDisabled} required maxLength={1} type="text" name="guess" id="guessed_letter" className="text-xl text-center border-1 border-white outline-none rounded-md min-w-[33px] w-[5%]" />
        <input disabled={!RadioDisabled} onClick={() => console.log("Submitted")} type="submit" value="Check" className={`transition-color duration-300 linear font-bold border-2 border-white pl-1 pr-1 rounded-md '${!RadioDisabled ? 'Opacity-50 cursor-not-allowed text-gray-400' : ' cursor-pointer hover:bg-white hover:text-black'}`} />
      </form>
    </div>
  );
}
